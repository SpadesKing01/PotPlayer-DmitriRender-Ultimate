...
1. **动态相对时间欺骗（核心突破）**：
   - 专用加载器动态注入时间，每次启动**恒定相对当前系统时间滞后 2 年（如 2024 年安全时钟窗口）**。
   - **时间随真实系统时钟同步流逝**，彻底杜绝静态死时间引发的 StarForce “检测到时钟倒流作弊” 与 2026 年硬边界锁死。
2. **后台静默续期计划任务**：
...
7. **合规 Shell 能力注册与官方高清图标**：
   - 自动写入 Windows `Capabilities`、`RegisteredApplications` 以及 `SupportedTypes`，完全符合现代 Windows 媒体应用规范。
   - 桌面快捷方式与打开方式菜单统一展示为 **PotPlayer 官方原生高清经典图标**（`PotPlayerMini64.exe,0`），杜绝第三方日历图标。
...
├── PotPlayerMini64.exe               # 自愈免续期启动加载器（集成原版高清图标）
├── PotPlayer64_Core.exe              # PotPlayer 官方 64 位核心原版程序（系统隐藏）
...
1. **默认播放器设置（首装或重装必看）**：
   - Windows 10/11 系统内置了内核级防篡改机制（`UCPD.sys`），严禁任何第三方程序通过后台脚本静默改写默认播放器（否则会导致系统开机重置并弹窗骚扰）。
   - **首次使用或卸载重装后**：只需**右键任意视频文件（如 `.mp4`） $\rightarrow$【打开方式】 $\rightarrow$【选择其他应用】 $\rightarrow$ 选中【PotPlayer】并勾选【始终使用此应用打开】**。系统仅需确认这一次，即可获得微软受信任的永久加密签名，后续重启电脑永久生效，绝不会被系统重置。
2. **后续版本无感升级覆盖**：
   - 当官方 PotPlayer 发布新版本时，**直接下载官方 64 位原版 `PotPlayerMini64.exe` 覆盖到本目录即可**。
   - 覆盖后**无需任何复杂操作**，只需以管理员身份运行一次【一键绿化安装.bat】，脚本会自动将新版设为系统隐藏核心，并保留免续期自愈加载器。
3. **日常播放行为**：
   - 无论是双击桌面快捷方式、双击关联的媒体文件、还是先打开播放器再拖入视频，均由 `PotPlayerMini64.exe` 透明调度进入安全的动态时间窗口（恒定比当前时间相对滞后 2 年至 2024 年），无需手动调用 RunAsDate。
4. **彻底卸载与还原**：
   - 如需迁移或彻底卸载，请以管理员身份运行【一键彻底卸载.bat】，会自动注销所有 DirectShow 滤镜、清理计划任务、并自动将隐藏核心还原为原版官方程序，不留任何系统垃圾。
5. **画质与插帧验证**：
   - 播放视频时按下键盘 **`Tab`** 键，查看输出帧率是否达到 60.00 fps，滤镜链中是否包含 `DmitriRender` 和 `LAV Audio Decoder`。
...
1. 终止残留播放器与后台守护进程；
2. 自动检测官方新版并部署自愈加载器；
3. 部署并注册 DmitriRender 滤镜至 `%APPDATA%\DmitriRender`；
4. 注册 64 位 LAV Filters（音频/视频/分离器）及 madVR；
5. 导入全套优化预设参数（硬件硬解、音轨优先、OSD禁用）；
6. 创建 Windows 后台静默续期任务（每 20 天下午 15:00 自动维护）；
7. 写入 Windows 默认媒体播放器能力集（Capabilities）与支持类型（SupportedTypes）；
8. 全局锁定 PotPlayer 官方高清图标并刷新 Explorer 图标缓存。
