...
- **一键全自动关联与默认播放器**：
  运行【一键绿化安装.bat】后，脚本已自动完成 Windows `Capabilities` 与媒体播放器认证注册，并将 `.mp4`、`.mkv`、`.avi`、`.flv` 等 20 种主流视频格式全量关联。**全部视频文件图标均已直接显示为 PotPlayer 官方原生高清图标**，彻底杜绝 RunAsDate 默认日历图标覆盖。
- **日常播放方式**：
  - 直接双击任意本地视频文件即可自动拉起带插帧保护的播放；
  - 也可随时双击桌面生成的 **【PotPlayer (插帧免续期版)】** 快捷方式打开播放。
- **若 Windows 首次弹出【选择打开方式 / 如何打开此文件？】**：
  1. 列表中推荐的第一项已自动命名为 **【PotPlayer (插帧免续期版)】**（部分旧系统若显示为 `RunAsDate`，其图标也已全部替换为 PotPlayer 官方图标）；
  2. 选中并点击 **【始终】**（或勾选【始终使用此应用打开】）即可永久秒开。
