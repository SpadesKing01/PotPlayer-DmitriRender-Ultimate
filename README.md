...
- **推荐启动方式**：
  直接双击桌面生成的 **【PotPlayer (插帧免续期版)】** 快捷方式，或将视频拖入播放器中。
- **双击视频弹出【选择打开方式 / 如何打开此文件？】弹窗时**：
  1. **必须选择**：直接选择默认推荐的第一个 **【RunAsDate】**（或显示为 **【PotPlayer (插帧免续期版)】**）；
  2. 务必勾选底部的 **【始终使用此应用打开 .xxx 文件】**（Windows 11 点击【始终】按钮）；
  3. **为什么选 RunAsDate？**
     因为 `RunAsDate.exe` 是外层动态时钟欺骗加载器，它的底层命令已自动配置好调起 PotPlayer 并传递视频路径。**选择 RunAsDate 才能启动带防锁死保护的插帧播放**；若手动选择原版 PotPlayer 反而会因缺少时间偏移参数导致插帧直接过期。
