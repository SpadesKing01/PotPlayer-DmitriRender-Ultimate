...
- **推荐启动方式**：
  直接双击桌面生成的 **【PotPlayer (插帧免续期版)】** 快捷方式，或拖入视频文件播放。
- **双击视频弹出【选择打开方式 / 如何打开此文件？】弹窗时**：
  1. 在应用列表中找到并选中带有官方黄色图标的 **【PotPlayer (64-bit)】**；
  2. 务必勾选底部的 **【始终使用此应用打开 .xxx 文件】**（Windows 11 为“始终”按钮）；
  3. **重要提示**：**切勿**点击“在电脑上查找其他应用”去直接选择根目录下的 `PotPlayerMini64.exe`。双击视频必须通过脚本注册的关联项（已内嵌 `RunAsDate` 动态时间参数）启动，否则会失去时钟欺骗保护导致插帧过期。
